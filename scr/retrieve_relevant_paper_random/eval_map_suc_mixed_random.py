...
logger = logging.getLogger('top_k_eval')
...
def one_fold_mixed():
    ...
    workbook = xlsxwriter.Workbook(
        path + 'eval_one_fold.mix.{}.{}.xlsx'.format(random_sample_size, top_relevant_paper_size))
            ...
            for info2 in paper_info:
                for feature_type2 in feature_types:
                    if info1 == info2 and feature_type1 == feature_type2:
                        continue
                    logger.info('train.cos.{}.{} mixed with train.cos.{}.{} ...'.format(info1,feature_type1, info2,feature_type2))
                    row += 1
                    for i, k in enumerate(k_list):
                        ws_avg_map_k.write(0, i + 1, 'avg_map@{}'.format(k))
                        ws_avg_map_k.write(row, 0, '{}.{}.mix.{}.{}'.format(info1, feature_type1, info2, feature_type2))
                        ws_var_map_k.write(0, i + 1, 'var_map@{}'.format(k))
                        ws_var_map_k.write(row, 0, '{}.{}.mix.{}.{}'.format(info1, feature_type1, info2, feature_type2))
                        ws_avg_suc_k.write(0, i + 1, 'avg_suc@{}'.format(k))
                        ws_avg_suc_k.write(row, 0, '{}.{}.mix.{}.{}'.format(info1, feature_type1, info2, feature_type2))
                        ws_var_suc_k.write(0, i + 1, 'avg_suc@{}'.format(k))
                        ws_var_suc_k.write(row, 0, '{}.{}.mix.{}.{}'.format(info1, feature_type1, info2, feature_type2))

                        predictions = pd.read_csv(path + 'train.cos.{}.{}.mix.{}.{}.{}.top{}'.format(info1, feature_type1, info2,feature_type2, random_sample_size, top_relevant_paper_size))
