...
# glove_file="../../Dataset/pretrained/glove.840B.300d.txt"
import sys
import traceback

base_path = '../../../../Dataset/pretrained/'
for glove_file, v in pretrain_checksum.iteritems():
    try:
        print glove_file

        parts = glove_file.split('.')
        if len(parts) > 4:
            tokens = parts[2]
            dimensions = parts[3]
        else:
        ...
        gensim_file = glove_file + '.gensim'
        gensim_first_line = "{} {}".format(num_lines, dims)

        glove_file = base_path + glove_file
        gensim_file = base_path + gensim_file
        # Prepends the line.
