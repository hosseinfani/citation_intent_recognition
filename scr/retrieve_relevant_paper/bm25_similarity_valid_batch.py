...
logger = logging.getLogger('bm25_valid')
...
if __name__ == '__main__':
    ...
    valid_corpus = [[word for word in text.split()] for text in valid['processed_description'][batch_idx[0]:batch_idx[1]].tolist()]
    ...
    train_scores = [bm.get_scores(text)[:n_papers] for text in valid_corpus]


    logger.info('saving bm scores ...')
    joblib.dump(np.asarray(train_scores), '../output/similarities/valid.bm25.{}.{}'.format(bath_size, batch_no), compress=3)
