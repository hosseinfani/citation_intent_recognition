...
logger = logging.getLogger('vector_sim_train')
...
def vector_sim(desc_feature_file, paper_feature_file, metric, output, sample_idx):
    ...
    sims = 1 - metrics.pairwise_distances(X=desc_feature_file[sample_idx,:], Y=paper_features, metric=metric, n_jobs=-1)
    ...
    for info in paper_info:
        for feature_type in feature_types:
            vector_sim(path + 'features/train.{}'.format(feature_type),
                       ...
                       'cosine',
                       path + 'similarities/train.cos.{}.{}.{}.{}'.format(info, feature_type, bath_size, batch_no),
                       batch_idx)
